...
const baseModule = require('../src/packExternalModules');
const packagerFactoryMock = require('../src/packagers/index');

jest.mock('fs-extra');
jest.mock('../src/packagers/index', () => {
  const packagerMock = {
...
const createStatsMock = (modules) => ({
  stats: [
    {
      outputPath: '/my/Service/Path/.rspack/service',
...
describe('packExternalModules', () => {
  ...
  beforeEach(() => {
    ...
    _.set(serverless, 'service.custom.rspackIncludeModules', true);
  ...
  describe('packExternalModules()', () => {
    ...
    const statsWithFileRef = createStatsMock([
      '@scoped/vendor',
      'uuid',
      'localmodule',
      'bluebird',
    ]);
    const statsWithDevDependency = createStatsMock([
      'eslint',
      '@scoped/vendor',
      'uuid',
      'localmodule',
    ...
    it('should do nothing if rspackIncludeModules is not set', () => {
    ...
    it('should copy needed package sections if available', () => {
      ...
      packagerFactoryMock
        .get('npm')
        .getProdDependencies.mockReturnValue(BbPromise.resolve({}));
      packagerFactoryMock
        .get('npm')
        .getPackagerVersion.mockReturnValue(BbPromise.resolve());
      packagerFactoryMock
        .get('npm')
        .install.mockReturnValue(BbPromise.resolve());
      packagerFactoryMock.get('npm').prune.mockReturnValue(BbPromise.resolve());
      packagerFactoryMock
        .get('npm')
        .runScripts.mockReturnValue(BbPromise.resolve());
      module.compileStats = stats;
      return expect(module.packExternalModules())
        .resolves.toBeUndefined()
        .then(() =>
          BbPromise.all([
            ...
            expect(writeFileSyncStub.mock.calls[1][1]).toEqual(
            ...
            expect(
              packagerFactoryMock.get('npm').getProdDependencies,
            ).toHaveBeenCalledTimes(1),
            expect(
              packagerFactoryMock.get('npm').getPackagerVersion,
            ).toHaveBeenCalledTimes(2),
            expect(
              packagerFactoryMock.get('npm').install,
            ).toHaveBeenCalledTimes(1),
            expect(packagerFactoryMock.get('npm').prune).toHaveBeenCalledTimes(
              1,
            ),
            expect(
              packagerFactoryMock.get('npm').runScripts,
            ).toHaveBeenCalledTimes(1),
          ]),
        );
    });
    ...
    it('should rebase file references', () => {
      ...
      packagerFactoryMock
        .get('npm')
        .rebaseLockfile.mockImplementation(
          (pathToPackageRoot, lockfile) => lockfile,
        );
      packagerFactoryMock
        .get('npm')
        .getPackagerVersion.mockReturnValue(BbPromise.resolve());
      ...
      sandbox
        .stub(process, 'cwd')
        .returns(path.join(path.sep, 'my', 'Service', 'Path'));
      jest.doMock(
      ...
      return expect(module.packExternalModules())
        ...
        .then(() =>
          BbPromise.all([
            ...
            expect(writeFileSyncStub.mock.calls[2][1]).toEqual(
              JSON.stringify(expectedPackageJSON, null, 2),
            ),
            ...
            expect(
              packagerFactoryMock.get('npm').rebaseLockfile,
            ).toHaveBeenCalledTimes(1),
            expect(
              packagerFactoryMock.get('npm').rebaseLockfile,
    ...
    it('should reject if packager install fails', () => {
      ...
      packagerFactoryMock
        ...
        .install.mockImplementation(() =>
          BbPromise.reject(new Error('npm install failed')),
        );
      packagerFactoryMock.get('npm').prune.mockReturnValue(BbPromise.resolve());
      packagerFactoryMock
        .get('npm')
    ...
    it('should reject if packager returns a critical error', () => {
      ...
      packagerFactoryMock
        .get('npm')
        .getProdDependencies.mockImplementation(() =>
          BbPromise.reject(new Error('something went wrong')),
        );
      module.compileStats = stats;
      return expect(module.packExternalModules())
    ...
    it('should report ignored packager problems in verbose mode', () => {
      ...
      return expect(module.packExternalModules())
        ...
        .then(() => {
          ...
          expect(logMock.verbose).toHaveBeenCalledWith('=> Problem 1');
          expect(logMock.verbose).toHaveBeenCalledWith('=> Problem 2');
          return null;
        });
    });
    ...
    it('should reject if devDependency is required at runtime', () => {
      ...
      return expect(module.packExternalModules())
        .rejects.toThrow('Serverless-rspack dependency error: eslint.')
        .then(() =>
          BbPromise.all([
            expect(logMock.error).toHaveBeenCalledWith(
              expect.stringMatching(
                /Runtime dependency 'eslint' found in devDependencies/,
              ),
            ),
            // npm ls and npm install should have been called
            expect(
        ...
        .then(() =>
          BbPromise.all([
            expect(logMock.verbose).toHaveBeenCalledWith(
              expect.stringMatching(
                /Runtime dependency 'aws-sdk' found in devDependencies/,
              ),
            ),
            // npm ls and npm install should have been called
            expect(
      ...
      return expect(module.packExternalModules())
        ...
        .then(() =>
          BbPromise.all([
            ...
            expect(readFileSyncStub.mock.calls[0][0]).toEqual(
              path.join(process.cwd(), './package.json'),
            ),
            expect(readFileSyncStub.mock.calls[1][0]).toEqual(
              path.join(process.cwd(), './mocked-lock.json'),
            ),
            // The module package JSON and the composite one should have been stored
            expect(writeFileSyncStub).toHaveBeenCalledTimes(3),
            expect(writeFileSyncStub.mock.calls[0][1]).toEqual(
            ...
            expect(writeFileSyncStub.mock.calls[1][1]).toEqual(
              JSON.stringify({ info: 'lockfile' }, null, 2),
            ),
            expect(writeFileSyncStub.mock.calls[2][1]).toEqual(
              JSON.stringify(expectedPackageJSON, null, 2),
            ),
    ...
    describe('peer dependencies', () => {
      ...
      describe('default behavior', () => {
        beforeAll(() => {
          jest.resetModules();
          jest.doMock(path.join('..', 'package.json'), () =>
        ...
        it('should install external peer dependencies', () => {
          ...
          packagerFactoryMock
            .get('npm')
            .prune.mockReturnValue(BbPromise.resolve());
          packagerFactoryMock
            .get('npm')
            .runScripts.mockReturnValue(BbPromise.resolve());
          ...
          return expect(module.packExternalModules())
            ...
            .then(() =>
              BbPromise.all([
                ...
                expect(
                  packagerFactoryMock.get('npm').prune,
                ).toHaveBeenCalledTimes(1),
                expect(
                  packagerFactoryMock.get('npm').runScripts,
                ).toHaveBeenCalledTimes(1),

...
const baseModule = require('../src/packageModules');
  ...
  describe('packageModules()', () => {
    ...
    describe('with service packaging', () => {
      ...
      describe('with the Google provider', () => {
        ...
        it('should set the service artifact path', () => {
          ...
          return expect(module.packageModules()).resolves.toEqual([
            path.join('.rspack', 'test-service.zip'),
          ]);
        });
      });
      ...
      it('should reject if no files are found', () => {
        ...
        module.compileStats = stats;
        return expect(module.packageModules()).rejects.toThrow(
    ...
    describe('with individual packaging', () => {
      ...
      const stats = {
        stats: [
          {
            outputPath: '/my/Service/Path/.rspack/func1',
          },
          {
            outputPath: '/my/Service/Path/.rspack/func2',
          },
        ],
      };
      const files = [
      ...
      it('should package', () => {
        ...
        return expect(module.packageModules()).resolves.toEqual([
          path.join('.rspack', 'func1.zip'),
          path.join('.rspack', 'func2.zip'),
        ]);
      });
    });
  });
  ...
  describe('copyExistingArtifacts()', () => {
    ...
    describe('with service packaging', () => {
      ...
      it('copies the artifact', () => {
        const expectedArtifactSource = path.join('.rspack', 'test-service.zip');
        const expectedArtifactDestination = path.join(
          '.serverless',
          'test-service.zip',
        );
          ...
          .then(() =>
            BbPromise.all([
              ...
              expect(fsMock.copyFileSync).toHaveBeenCalledWith(
                ...
                expectedArtifactDestination,
              ),

              // Should set package artifact for each function to the single artifact
              expect(func1).toHaveProperty(
                'package.artifact',
                expectedArtifactDestination,
              ),
              expect(func2).toHaveProperty(
                'package.artifact',
                expectedArtifactDestination,
              ),
            ]),

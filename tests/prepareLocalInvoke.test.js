...
const baseModule = require('../src/prepareLocalInvoke');
...
describe('prepareLocalInvoke', () => {
  ...
  it('should store original service path', () => {
    ...
    module.serverless.service.getFunction.mockReturnValue({
      handler: 'myFuncHandler',
    });
  ...
  it('should use the function folder as cwd', () => {
    ...
    module.rspackOutputPath = '.rspack';
    module.serverless.config.servicePath = './servicePath';
    module.prepareLocalInvoke();

    expect(process.chdir.args[0]).toEqual([path.join('.rspack', 'myFunc')]);
  });
  ...
  it('should use the service folder as cwd', () => {
    ...
    expect(process.chdir.args[0]).toEqual([path.join('.rspack', 'service')]);
  });

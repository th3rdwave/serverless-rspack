...
describe('wpwatch', () => {
  ...
  beforeEach(() => {
    jest.resetModules();
    rspackMock = require('@rspack/core');
    baseModule = require('../src/wpwatch');
    ...
    _.set(module, 'rspackConfig', rspackConfig);
  });

  it('should reject if rspack watch fails', () => {
    const wpwatch = module.wpwatch.bind(module);
    rspackMock.compilerMock.watch.mockReset();
    rspackMock.compilerMock.watch.mockImplementation((options, cb) =>
      cb(new Error('Failed')),
    );
  ...
  it('should spawn compile if watch is disabled', () => {
    ...
    _.set(module.options, 'rspack-no-watch', true);

    return expect(wpwatch())
      .resolves.toBeUndefined()
      .then(() =>
        BbPromise.join(
          expect(spawnStub).toHaveBeenCalledWith('rspack:compile'),
          expect(rspackMock.compilerMock.watch).toHaveBeenCalledTimes(0),
        ),
      );
  });
    ...
    return expect(wpwatch())
      ...
      .then(() =>
        BbPromise.join(
          ...
          expect(rspackMock.compilerMock.watch).toHaveBeenCalledTimes(1),
        ),
      );
  });
  ...
  it('should work if no stats are returned', () => {
    const wpwatch = module.wpwatch.bind(module);
    rspackMock.compilerMock.watch.mockReset();
    rspackMock.compilerMock.watch.mockImplementation((options, cb) => cb());
    spawnStub.mockResolvedValue();
  ...
  it('should enable polling with command line switch', () => {
    ...
    rspackMock.compilerMock.watch.mockImplementation((options, cb) => cb());
    spawnStub.mockResolvedValue();
    _.set(module.options, 'rspack-use-polling', true);
    ...
    return expect(wpwatch())
      ...
      .then(() =>
        BbPromise.join(
          expect(spawnStub).toHaveBeenCalledTimes(0),
          expect(rspackMock.compilerMock.watch).toHaveBeenCalledTimes(1),
          expect(rspackMock.compilerMock.watch).toHaveBeenCalledWith(
            {
              poll: 3000,
            },
            expect.any(Function),
  ...
  it('should set specific polling interval if given with switch', () => {
    ...
    rspackMock.compilerMock.watch.mockImplementation((options, cb) => cb());
    spawnStub.mockResolvedValue();
    _.set(module.options, 'rspack-use-polling', 5000);
  ...
  it('should spawn rspack:compile:watch on subsequent runs', () => {
    const wpwatch = module.wpwatch.bind(module);
    ...
    rspackMock.compilerMock.hooks.beforeCompile.tapPromise.mockImplementation(
      (options, cb) => {
        beforeCompileCallback = cb;
      },
    );

    rspackMock.compilerMock.watch.mockImplementationOnce((options, cb) => {
    ...
    return expect(wpwatch())
      ...
      .then(() =>
        BbPromise.join(
          ...
          expect(spawnStub).toHaveBeenCalledWith('rspack:compile:watch'),
        ),
      );
  });

  it('should spawn more rspack:compile:watch when previous is resolved', () => {
    const wpwatch = module.wpwatch.bind(module);
    let watchCallbackCount = 0;
    let beforeCompileCallback;
  ...
  it("should use plugins for rspack:compile:watch if hooks doesn't exist", () => {
    const wpwatch = module.wpwatch.bind(module);
    rspackMock.compilerMock.hooks = false;

    rspackMock.compilerMock.plugin.mockImplementation((name, cb) =>
      cb(null, _.noop),
    );
    rspackMock.compilerMock.watch.mockImplementation((options, cb) =>
      cb(null, {}),
    ...
    return expect(wpwatch())
      .resolves.toBeUndefined()
      .then(() =>
        expect(rspackMock.compilerMock.plugin).toHaveBeenCalledTimes(1),
      );
  });

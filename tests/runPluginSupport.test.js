...
const baseModule = require('../src/runPluginSupport');

const pluginPath = path.join('plugins', 'run', 'utils');
const deployFunctionsToLocalEmulatorStub = require(path.join(
  pluginPath,
...
jest.mock(
  'plugins/run/utils/deployFunctionsToLocalEmulator',
  () => jest.fn().mockResolvedValue(),
  { virtual: true },
);
jest.mock('plugins/run/utils/getLocalRootUrl', () => jest.fn(), {
  virtual: true,
});
// Support windows
jest.mock(
...
describe('runPluginSupport', () => {
      ...
      return expect(prepareRun())
        ...
        .then(() =>
          BbPromise.join(
            ...
            expect(module.originalRspackOutputPath).toEqual(rspackOutputPath),
            expect(module.keepOutputDirectory).toBe(true),
            expect(serverless.config.servicePath).toEqual(
              path.join(rspackOutputPath, 'service'),
            ),
  ...
  describe('watchRun', () => {
    beforeEach(() => {
      _.set(module, 'rspackConfig.output.path', 'outputPath');
    });
      ...
      return expect(watchRun())
        ...
        .then(() =>
          BbPromise.join(
            ...
            expect(deployFunctionsToLocalEmulatorStub).toHaveBeenCalledWith(
              service,
              undefined,
              undefined,
            ),

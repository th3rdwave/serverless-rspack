...
function setArtifactPath(funcName, func, artifactPath) {
  ...
  log.verbose(
    `Setting artifact for function '${funcName}' to '${artifactPath}'`,
  );
  ...
  return new BbPromise((resolve, reject) => {
    ...
    output.on('open', () => {
      ...
      const normalizedFiles = _.uniq(
        _.map(files, (file) => path.normalize(file)),
      );

      BbPromise.all(
        _.map(normalizedFiles, (file) =>
          getFileContentAndStat.call(this, directory, file),
        ),
      )
        .then((contents) => {
          _.forEach(
...
function zip(directory, zipFileName) {
  ...
  if (this.configuration.excludeRegex) {
    ...
    files = _.filter(
      files,
      (f) => f.match(this.configuration.excludeRegex) === null,
    );
    log.verbose(
      `Excluded ${
        existingFilesLength - files.length
      } file(s) based on excludeRegex`,
    );
  }

  if (_.isEmpty(files)) {
    const error = new this.serverless.classes.Error(
      'Packaging: No files found',
    );
  ...
  // This allows us to persist the rspackOutputPath and re-use the compiled output
  const artifactFilePath = path.join(this.rspackOutputPath, zipFileName);
...
function getArtifactLocations(name) {
  ...
  const rspackArtifact = path.join(this.rspackOutputPath, archiveName);
  const serverlessArtifact = path.join('.serverless', archiveName);

  return { rspackArtifact, serverlessArtifact };
}

function copyArtifactByName(artifactName) {
  const { rspackArtifact, serverlessArtifact } = getArtifactLocations.call(
    this,
    artifactName,
  );
  ...
  fs.copyFileSync(rspackArtifact, serverlessArtifact);
}
...
function getArtifactName(entryFunction) {
  return `${
    entryFunction.funcName || this.serverless.service.getServiceObject().name
  }.zip`;
}
...
module.exports = {
  packageModules() {
    ...
    log.verbose('[Rspack] Packaging modules');
    progress.get('rspack').notice('[Rspack] Packaging modules');
  ...
  copyExistingArtifacts() {
    log.verbose('[Rspack] Copying existing artifacts');
    progress.get('rspack').notice('[Rspack] Copying existing artifacts');
    ...
    if (individualPackagingEnabled) {
      _.forEach(functionNames, (funcName) =>
        copyArtifactByName.call(this, funcName),
      );
    } else {
    ...
    _.forEach(functionNames, (funcName) => {
      ...
      const { serverlessArtifact } = getArtifactLocations.call(
        this,
        archiveName,
      );

 ...
 */
function removeExcludedModules(modules, packageForceExcludes, shouldLog) {
  ...
  if (shouldLog && !_.isEmpty(excludedModules)) {
    log(`Excluding external modules: ${_.join(excludedModules, ', ')}`);
  }
}
  ...
  _.forEach(externalModules, (module) => {
    ...
    if (moduleVersion) {
      ...
      if (nodeModulesRelativeDir) {
        const customNodeModulesDir = path.join(
          process.cwd(),
          nodeModulesRelativeDir,
          'node_modules',
        );
        ...
        if (fse.pathExistsSync(customNodeModulesDir)) {
          ...
          log.warning(
            `${customNodeModulesDir} dose not exist. Please check nodeModulesRelativeDir setting`,
          );
        }
      }
      ...
      try {
        ...
        if (!_.isEmpty(peerDependencies)) {
          ...
          const peerDependenciesMeta =
            require(modulePackagePath).peerDependenciesMeta;

          if (!_.isEmpty(peerDependenciesMeta)) {
            _.forEach(peerDependencies, (value, key) => {
              if (
                peerDependenciesMeta[key] &&
      ...
      if (
        ...
        const originInfo =
          _.get(dependencyGraph, 'dependencies', {})[module.origin] || {};
        moduleVersion = _.get(
          _.get(originInfo, 'dependencies', {})[module.external],
          'version',
        );
        if (!moduleVersion) {
          moduleVersion = _.get(dependencyGraph, [
            'dependencies',
            module.external,
            'version',
          ]);
        ...
        if (!moduleVersion) {
          log.warning(
            `Could not determine version of module ${module.external}`,
          );
        }
        prodModules.push(
          moduleVersion
        ...
        if (!_.includes(ignoredDevDependencies, module.external)) {
          ...
          throw new this.serverless.classes.Error(
            `Serverless-rspack dependency error: ${module.external}.`,
          );
        }
        log.verbose(
...
module.exports = {
  ...
  packExternalModules() {
    ...
    log.verbose('Packing external modules');
    progress.get('rspack').notice('[Rspack] Packing external modules');
    ...
    return BbPromise.try(() =>
      Packagers.get.call(this, this.configuration.packager),
    ).then((packager) => {
      ...
      if (!_.isEmpty(packageSections)) {
        log.verbose(
          `Using package.json sections ${_.join(
            _.keys(packageSections),
            ', ',
          )}`,
        );
      }
      ...
      log.verbose(`Fetch dependency graph from ${packageJsonPath}`);
      ...
      return packager
        ...
        .then((dependencyGraph) => {
          ...
          if (this.options.verbose && !_.isEmpty(problems)) {
            log.verbose(`Ignoring ${_.size(problems)} NPM errors:`);
          ...
          const compositeModulePath = path.join(
            this.rspackOutputPath,
            'dependencies',
          );
          const compositePackageJson = path.join(
            compositeModulePath,
            'package.json',
          );
          ...
          const packageLockPath = path.join(
            findWorkspaceRoot(path.dirname(packageJsonPath)) ||
              path.dirname(packageJsonPath),
            packagerOptions.lockFile || packager.lockfileName,
          );
          let hasPackageLock = false;
          return BbPromise.fromCallback((cb) =>
            fse.pathExists(packageLockPath, cb),
          )
            .then((exists) => {
              if (exists) {
                ...
                try {
                  let packageLockFile =
                    this.serverless.utils.readFileSync(packageLockPath);
                  packageLockFile = packager.rebaseLockfile(
                    relPath,
                    packageLockFile,
                  );
            ...
            .then(() => {
              ...
              return packager
                .getPackagerVersion(compositeModulePath)
                .then((version) => {
                  return packager
                    .install(
                      compositeModulePath,
            ...
            .mapSeries((compileStats) => {
              ...
              return BbPromise.try(() => {
                ...
                .tap(() => {
                  log.verbose(
                    `Copy modules: ${modulePath} [${_.now() - startCopy} ms]`,
                  );
                })
                .then(() => {
                  ...
                  return packager
                    ...
                    .then((version) => {
                      return packager
                        ...
                        .tap(() => {
                          log.verbose(
                            `Prune: ${modulePath} [${_.now() - startPrune} ms]`,
                          );
                        });
                    });
                })
                .then(() => {
                  ...
                  return packager
                    ...
                    .tap(() => {
                      log.verbose(
                        `Run scripts: ${modulePath} [${
                          _.now() - startRunScripts
                        } ms]`,
                      );
                    });
                });

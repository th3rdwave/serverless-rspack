...
class Configuration {
  constructor(custom) {
    ...
    if (custom) {
      if (custom.rspackIncludeModules) {
        this._config.includeModules = custom.rspackIncludeModules;
        this._hasLegacyConfig = true;
      }
      if (_.isString(custom.rspack)) {
        this._config.rspackConfig = custom.rspack;
        this._hasLegacyConfig = true;
      } else {
        _.assign(this._config, custom.rspack || {});
      }
    }
  ...
  get rspackConfig() {
    return this._config.rspackConfig;
  }
